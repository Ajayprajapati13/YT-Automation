"""15-30s representative test of the shot-driven architecture.

Renders ONE scene (cpu_vs_gpu) from the existing GPU explainer as
independently-rendered shots (bounded per-shot ffmpeg processes, each
skippable/resumable), concatenated and muxed with narration synthesized
via the existing, unmodified TTS/timing path
(create_gpu_explainer.synthesize_scene_narration/compute_timeline).

Output:
  output/test_shot_driven_15_30s.mp4
  output/test_shot_plan.json
"""
...
BEATS_FILE = gpu.BASE_DIR / "content" / "gpu_explainer_beats.json"
TEST_TEMP_DIR = gpu.BASE_DIR / "temp" / "shot_driven_test"
...
def main() -> dict:
    ...
    beats = beats_data["scenes"][TEST_SCENE_ID]

    plan = shot_planner.build_beat_shots(scene, beats)
    shot_planner.write_beat_shot_plan(plan, SHOT_PLAN_OUT)
    print(f"Shot plan written: {SHOT_PLAN_OUT} ({len(plan['shots'])} shots)")
    for s in plan["shots"]:
        print(f"  {s['shot_id']:<32} {s['visual_strategy']:<26} {s['duration']:5.2f}s")
